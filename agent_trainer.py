...
def classless_trainer(
    ...
    def generate_players(game_count: int) -> (Player, Player):
        if training_method == 0:
            if game_count % 2 == 0:
                ...
                black_player = ML_Player(0, piece_encoder, score_encoder, rewards_boosting, model)
                #black_player = ML_Distiller(0, piece_encoder,score_encoder, rewards_boosting, model,depth=search_depth)
                # print("ML is black")
            else:
                white_player = ML_Player(1, piece_encoder, score_encoder, rewards_boosting, model)
            ...
            if game_count % 2 == 0:
                ...
                white_player = ML_Player(1, piece_encoder,score_encoder, rewards_boosting, model,epsilon=epsilon)
                black_player = ML_Player_Trainer(0, piece_encoder,score_encoder, rewards_boosting, old_model)
            else:
                white_player = ML_Player_Trainer(1, piece_encoder,score_encoder, rewards_boosting, old_model)
                #black_player = ML_Distiller(0, piece_encoder, rewards_boosting, model,epsilon=epsilon, depth=search_depth)
                black_player = ML_Player(0, piece_encoder,score_encoder, rewards_boosting, model,epsilon=epsilon)
            ...
            if game_count % 2 == 0:
                white_player = ML_Distiller(1, piece_encoder,score_encoder, rewards_boosting, model,depth=search_depth)
                black_player = ML_Player(0, piece_encoder,score_encoder, rewards_boosting, old_model,epsilon=epsilon)
            else:
                white_player = ML_Player(1, piece_encoder,score_encoder, rewards_boosting, old_model,epsilon=epsilon)
                black_player = ML_Distiller(0, piece_encoder,score_encoder, rewards_boosting, model,depth=search_depth)

        elif training_method == 5:
            if game_count % 2 == 0:
                white_player = chess_engine(1, piece_encoder,score_encoder, rewards_boosting,chosen_engine,0.01,chosen_name)
                black_player = ML_Player(0, piece_encoder,score_encoder, rewards_boosting, model)
                ...
                black_player = chess_engine(0, piece_encoder,score_encoder, rewards_boosting,chosen_engine,0.01,chosen_name)
    ...
    def train(updates=False):
        ...
        while True:
            ...
            #
            #     elif type(black_player) == ML_Distiller:
...
if __name__ == "__main__":
    ...
    try:
        my_q = Queue()
        set_up = input("Perform first time setup? (May error if requisite files are missing) Y/N\n")
        if set_up.lower() == 'y':
        ...
        verbosity = int(input("Verbose mode enabled? (1: yes, 0: no): \n"))
        print(f"Running training method {selected_method} with {num_processes} processes")
        if selected_method not in [1,6]:
            os.environ["CUDA_VISIBLE_DEVICES"] = "-1"
        fo_pool = [Process(target=classless_trainer, args=(my_q, selected_method,False,verbosity),daemon=True) for i in range(num_processes)]
        for p in fo_pool:
            p.start()
