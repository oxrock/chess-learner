...
def load_all_models(encoder,env):
    return [load_model(f"chess_model-trainer_{x+1}.mdl",encoder,env) for x in range(get_tracker_data("model_tracker.txt"))]
    ...
    for i in range(len(models)):
        for j in range(i+1,len(models)):
            for k in range(2):
                players = [i+1,j+1]
                if k%2 == 0:
                    player_white = ML_Player_Trainer(1, piece_encoder,score_encoder, False, models[i])
                    player_black = ML_Player_Trainer(0, piece_encoder,score_encoder, False, models[j])
                else:
                    players = [j+1, i+1]
                    player_white = ML_Player_Trainer(1, piece_encoder,score_encoder, False, models[j])
                    player_black = ML_Player_Trainer(0, piece_encoder,score_encoder, False, models[i])
        ...
        print(f"model {i+1} has finshed thier games with a score of {model_tallies[i+1]}")
