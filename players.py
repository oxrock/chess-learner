...
class Player:
    ...
    def finalize_data(self, reward: int) -> list:
        ...
        for i in range(len(self.history)):
            turn_reward = -inf
            base_reward = reward / (len(self.history) - i)
            if self.boosted_rewards and reward > 0:
...
class ML_Player(Player):
    def __init__(
        self, team: int, encoder: dict, boosted_rewards: bool, model, epsilon=0.9
    ):
...
class ML_Player_Trainer(ML_Player):
    def __init__(
        self, team: int, encoder: dict, boosted_rewards: bool, model, epsilon=0.95
    ):
        super().__init__(team, encoder, boosted_rewards, model, epsilon)
